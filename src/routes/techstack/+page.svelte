...
<svelte:head>
    <title>Froilan | TechStack</title>
</svelte:head>

<main class="py-10 w-11/12 max-w-7xl gap-2 flex flex-col mx-auto">
    <a
        href="/"
    ...
    <div class="flex flex-col gap-5">
        <h1 class="text-2xl text-center font-bold">TechStack</h1>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            {#each [
                { title: 'Frontend', items: FRONTEND },
                { title: 'Backend', items: BACKEND },
                { title: 'DevOps & Cloud', items: DEVOPS },
                { title: 'Developer Tools', items: DEVELOPER_TOOLS }
            ] as section, index}
                <div
                    in:fly={{ y: 20, duration: 300, delay: (index + 1) * 100 }}

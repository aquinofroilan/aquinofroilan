'use server';
import { CREDLY_USERNAME } from '$env/static/private';
import { CertificationsListsPreview } from '$lib/data/certification-list';
...
function isCredlyBadge(data: unknown): data is CredlyBadge {
	const d = data as Partial<CredlyBadge> | null | undefined;
	return (
		!!d &&
		typeof d.id === 'string' &&
		!!d.badge_template &&
		!!d.issuer &&
		Array.isArray(d.issuer.entities)
	);
}
...
export const getCredlyBadges = async () => {
	const now = Date.now();
	if (cachedBadges && now - lastFetch < 3600 * 1000) {
		return cachedBadges;
	}
	cachedBadges = await fetchCredlyBadges();
	lastFetch = now;
	return cachedBadges;
};
...
function shouldUseCredlyUrl(data: CredlyBadge): boolean {
	...
	return issuerNames.some((name) =>
		credlyIndicators.some((indicator) => name.includes(indicator))
	);
}
...
export async function getCredlyCertifications(): Promise<FormattedCertification[]> {
	try {
		...
		const formattedCredlyBadges = credlyBadges.map((badge) =>
			transformCertification(badge, true)
		);
